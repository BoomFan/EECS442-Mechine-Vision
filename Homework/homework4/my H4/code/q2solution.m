...
% 5. Determine shreshold by multiply the scale with a factor $k$. \\
...
radius = [];
for i=1:levels
    [rows,cols] = find(scale_space_filtered(:,:,i)>=T);
    num = length(rows);
    radis = sigma(i)*sqrt(2);
    radis = repmat(radis,num,1);
    row = [row;rows];
    col = [col;cols];
    radius = [radius;radis];
end
...
% title(sprintf('%d circles, threshold %f', size(col,1), T));
